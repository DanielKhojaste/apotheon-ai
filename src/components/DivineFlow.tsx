  ...
  useEffect(() => {
    ...
    function pointOnStream(stream: Stream, u: number, time: number) {
      ...
      const slowWave = Math.sin(
        u * TAU * 1.15 + time * stream.speed + stream.phase,
      );
      const fineWave = Math.sin(
        u * TAU * 2.2 - time * stream.speed * 0.55 + stream.phase * 0.7,
      );
      y += (slowWave * stream.waveAmp + fineWave * stream.waveAmp * 0.28) *
        envelope * motion;

      // Very subtle horizontal breathing keeps the ribbons from looking like SVG rails.
      x +=
        Math.cos(u * TAU + time * 0.28 + stream.phase) *
        2.2 *
        envelope *
        motion;
    ...
    function drawStream(stream: Stream, time: number) {
      ...
      ctx.strokeStyle = `rgba(208, 159, 75, ${stream.alpha * 0.22})`;
      ctx.lineWidth = stream.width * 5.5;
      ctx.shadowColor = 'rgba(255, 190, 86, .44)';
      ctx.shadowBlur = 15;
      ctx.stroke();
      ...
      ctx.strokeStyle = `rgba(244, 205, 128, ${stream.alpha})`;
